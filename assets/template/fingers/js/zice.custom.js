...
function confirmSubmit(){	
    ...
    function validateSteps(step){
    	...
    	var prodi = 0;
    	var jenis = $('#jenis').val();    	
    	$(".utama"+step).each(function() {
		  var nilai = $(this).val();
		  if(!isNaN(parseFloat(nilai)) && isFinite(nilai)){
		  	total += parseInt(nilai);
		  }		  
		});   
		$(".anak"+step).each(function() {
		  var nilai = $(this).val();		  
		  if(!isNaN(parseFloat(nilai)) && isFinite(nilai)){
		  	anak += parseInt(nilai);
		  }		  
		});   
		$(".prodi"+step).each(function() {			
		  var nilai = $(this).val();		  
		  if(!isNaN(parseFloat(nilai)) && isFinite(nilai)){
		  	prodi += parseInt(nilai);
		  }		  
		});   				
		if(total != anak && anak != 0){			
			$('#pesan').html('<div class="alertMessage SE">okelah kalo begitu'+total+' dan '+anak+'</div>');
			isStepValid = false;   
			return false;
		}		
		if(total != prodi && prodi != 0){			
			$('#pesan').html('<div class="alertMessage SE">Janganlah kamu begitu</div>');
			isStepValid = false;   
			return false;
		}		
        $.post( "siswa/"+jenis+"/1", $( "#wizardsss"+step ).serialize(), function( data ) {
			if(data == 'ok'){
				isStepValid = true;   
			}else{
				isStepValid = false;   
...
$(document).ready(function(){	
	...
	$('.alluppercase input').autotab_magic().autotab_filter({ format: 'custom', uppercase: true, pattern: '[^0-9a-zA-Z\. ]' });
	$('.bilangan input').autotab_magic().autotab_filter({ format: 'custom', pattern: '[^0-9,]' });
	$('.bilangane input').autotab_magic().autotab_filter({ format: 'custom', pattern: '[^0-9]' });
...
$(function() {		
   ...
   $('#timepicker').timepicker({});
	...
	$('.logout').live('click',function() { 
		  ...
		  loading(str,overlay);
		  setTimeout("unloading()",1500);
		  setTimeout( "window.location.href='home/logout'", 2000 );
	  });
