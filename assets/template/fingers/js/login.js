...
function Login(){	
	...
	$.ajax({
		...
		type: "POST",
	  	...
	  	url: site+'home/logon/'+user+'/'+token,

	  	data: $( "#formLogin" ).serialize(),

	  	success: function(response){	  		
	  		if(response.statuss == 'ok'){
	  			...
	  			if(response.level == 100 || response.level == 101){
	  				setTimeout( "window.location.href='"+site+"paud/dashboard'", 3000 );
	  			}else{
	  				setTimeout( "window.location.href='"+site+"dashboard/index/"+user+"/"+token+"'", 3000 );
	  			}
...
function showError(str){
	...
	$('#alertMessage').addClass('error').html(str.statuss).stop(true,true).show().animate({ opacity: 1,right: '10'}, 500);	
...
function showSuccess(str){
	...
	$('#alertMessage').removeClass('error').html(str.statuss).stop(true,true).show().animate({ opacity: 1,right: '10'}, 500);	
