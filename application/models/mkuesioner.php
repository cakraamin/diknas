...
class Mkuesioner extends CI_Model{
	...
	function getListKuesioner($ta,$tingkat,$kode,$jenjang)
	{
		...
		$sql = "SELECT * FROM kuesioner a,ket_kuesioner b,detail_kuesioner c WHERE a.id_kuesioner=b.id_kuesioner AND b.id_ket_kuesioner=c.id_ket_kuesioner AND c.jenjang_school='$tingkat' AND b.stat_ket_kuesioner='1' AND c.detail_jenjang_kuesioner='$jenjang' AND a.jawaban=1 ORDER BY b.id_ket_kuesioner";
		$kueri = $this->db->query($sql);
		$kueris = $kueri->result();
