...
class Mmapel extends CI_Model{
	...
	function getListMapelLain($ta,$kode)
	{
		$kueri = $this->db->query("SELECT * FROM rata_mapel WHERE id_ta='$ta' AND id_school='$kode'");
		$data = $kueri->row();
		$hasil = (isset($data->rata_nilai))?$data->rata_nilai:"";
		return $hasil;
	}
	...
	function addDetailMapelLain($tahun)
	{
		$data = array(
			'id_rata_mapel'				=> '',
			'id_ta'						=> $tahun,
			'id_school'					=> $this->session->userdata('id_school'),			
			'rata_nilai'				=> $this->input->post('nilai_rata')
		);

		$this->db->insert('rata_mapel', $data); 
	}
}
?>