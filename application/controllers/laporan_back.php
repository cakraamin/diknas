...
class Laporan extends CI_Controller {
	...
	function index()
	{
		$data = array(
			...
			'kecamatan'		=> $this->mlaporan->getKecamatanSelek()
		);
	...
	function generate_spm()
	{
		...
		foreach($kecamatan as $dt_kecamatan)
		{
			...
			$jumlah = $this->mlaporan->getJumlahSkul($dt_kecamatan->id_kecamatan);
			$this->excel->getActiveSheet()->setCellValue('C'.$baris, $jumlah[1]);
			$this->excel->getActiveSheet()->setCellValue('D'.$baris, $jumlah[2]);
			$this->excel->getActiveSheet()->setCellValue('E'.$baris, $jumlah[3]);
			$this->excel->getActiveSheet()->setCellValue('F'.$baris, $jumlah[4]);
			$sd = $sd + $jumlah[1];
			$smp = $smp + $jumlah[2];
			$sma = $sma + $jumlah[3];
			$smk = $smk + $jumlah[4];
		...
		$this->excel->getActiveSheet()->setCellValue('A'.$baris, "JUMLAH");
		$this->excel->getActiveSheet()->getStyle('A'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$this->excel->getActiveSheet()->setCellValue('C'.$baris, $sd);
		$this->excel->getActiveSheet()->setCellValue('D'.$baris, $smp);
		$this->excel->getActiveSheet()->setCellValue('E'.$baris, $sma);
		$this->excel->getActiveSheet()->setCellValue('F'.$baris, $smk);

		$this->excel->getActiveSheet()->getStyle('A3:F'.$baris)->applyFromArray($styleArray);
		...
		$this->excel->getActiveSheet()->getStyle('C4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('D4', 'CPNS');
		$this->excel->getActiveSheet()->getStyle('D4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('E4', 'GTT');
		$this->excel->getActiveSheet()->getStyle('E4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('F4', 'PTT');
		$this->excel->getActiveSheet()->getStyle('F4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('G4', 'GTY');
		$this->excel->getActiveSheet()->getStyle('G4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('H4', 'PTY');
		...
		foreach($kecamatan as $dt_kecamatan)
		{
			...
			$this->excel->getActiveSheet()->setCellValue('C'.$baris, $detailpeg['pns']);
			$this->excel->getActiveSheet()->setCellValue('D'.$baris, $detailpeg['cpns']);
			$this->excel->getActiveSheet()->setCellValue('E'.$baris, $detailpeg['gtt']);
			$this->excel->getActiveSheet()->setCellValue('F'.$baris, $detailpeg['ptt']);
			$this->excel->getActiveSheet()->setCellValue('G'.$baris, $detailpeg['gty']);
			$this->excel->getActiveSheet()->setCellValue('H'.$baris, $detailpeg['pty']);			
			$total = $detailpeg['pns'] + $detailpeg['cpns'] + $detailpeg['gtt'] + $detailpeg['ptt'] + $detailpeg['gty'] + $detailpeg['pty'];
			$this->excel->getActiveSheet()->setCellValue('I'.$baris, $total);			
			$pns = $pns + $detailpeg['pns'];
			$cpns = $cpns + $detailpeg['cpns'];
			$gtt = $gtt + $detailpeg['gtt'];
			$ptt = $ptt + $detailpeg['ptt'];
			$gty = $gty + $detailpeg['gty'];
			$pty = $pty + $detailpeg['pty'];
			$all = $all + $total;
			$no++;
			$baris++;
		}
		...
		$this->excel->getActiveSheet()->setCellValue('A'.$baris, 'JUMLAH');
		$this->excel->getActiveSheet()->getStyle('A'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$this->excel->getActiveSheet()->setCellValue('C'.$baris, $pns);
		$this->excel->getActiveSheet()->setCellValue('D'.$baris, $cpns);
		$this->excel->getActiveSheet()->setCellValue('E'.$baris, $gtt);
		$this->excel->getActiveSheet()->setCellValue('F'.$baris, $ptt);
		$this->excel->getActiveSheet()->setCellValue('G'.$baris, $gty);
		$this->excel->getActiveSheet()->setCellValue('H'.$baris, $pty);
		$this->excel->getActiveSheet()->setCellValue('I'.$baris, $all);

		$this->excel->getActiveSheet()->getStyle('A3:I'.$baris)->applyFromArray($styleArray);
		...
		$baris1 = $baris + 1;
		$awal2 = $baris;
		$this->excel->getActiveSheet()->mergeCells('A'.$baris.':A'.$baris1);
		$this->excel->getActiveSheet()->setCellValue('A'.$baris, 'NO');
		$this->excel->getActiveSheet()->getStyle('A'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->getStyle('A'.$baris)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('B'.$baris.':B'.$baris1);
		...
		foreach($kecamatan as $dt_kecamatan)
		{
			...
			$this->excel->getActiveSheet()->mergeCells('C'.$baris.':D'.$baris);
			$this->excel->getActiveSheet()->setCellValue('C'.$baris, $sertifikasi['pns_ser']);
			$this->excel->getActiveSheet()->mergeCells('E'.$baris.':F'.$baris);
			$this->excel->getActiveSheet()->setCellValue('E'.$baris, $sertifikasi['non_pns_ser']);			
			$this->excel->getActiveSheet()->mergeCells('G'.$baris.':H'.$baris);
			$this->excel->getActiveSheet()->setCellValue('G'.$baris, $sertifikasi['pns_bel']);			
			$this->excel->getActiveSheet()->mergeCells('I'.$baris.':J'.$baris);
			$this->excel->getActiveSheet()->setCellValue('I'.$baris, $sertifikasi['non_pns_bel']);
			$pns_ser = $pns_ser + $sertifikasi['pns_ser'];
			$non_pns_ser = $non_pns_ser + $sertifikasi['non_pns_ser'];
			$pns_bel = $pns_bel + $sertifikasi['pns_bel'];
			$non_pns_bel = $non_pns_bel + $sertifikasi['non_pns_bel'];
			$no++;
			$baris++;
		}
		$this->excel->getActiveSheet()->mergeCells('A'.$baris.':B'.$baris);
		...
		$this->excel->getActiveSheet()->getStyle('A'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$this->excel->getActiveSheet()->mergeCells('C'.$baris.':D'.$baris);
		$this->excel->getActiveSheet()->setCellValue('C'.$baris, $pns_ser);
		$this->excel->getActiveSheet()->mergeCells('E'.$baris.':F'.$baris);
		$this->excel->getActiveSheet()->setCellValue('E'.$baris, $non_pns_ser);			
		$this->excel->getActiveSheet()->mergeCells('G'.$baris.':H'.$baris);
		$this->excel->getActiveSheet()->setCellValue('G'.$baris, $pns_bel);			
		$this->excel->getActiveSheet()->mergeCells('I'.$baris.':J'.$baris);
		$this->excel->getActiveSheet()->setCellValue('I'.$baris, $non_pns_bel);		
	...
	function generate_rk1()
	{
		...
		$kolomsss = 1+(count($umur)*2)+($tingkat*2);
		...
		{
			$this->excel->getActiveSheet()->mergeCells('C3:F3');
			$this->excel->getActiveSheet()->setCellValue('C3', 'PENDAFTAR');
			$this->excel->getActiveSheet()->getStyle('C3')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->excel->getActiveSheet()->mergeCells('C4:D4');
			$this->excel->getActiveSheet()->setCellValue('C4', 'TAMATAN TK');
			$this->excel->getActiveSheet()->getStyle('C4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->excel->getActiveSheet()->setCellValue('C5', 'L1');
			...
			$this->excel->getActiveSheet()->setCellValue('E4', 'BUKAN TK');
			$this->excel->getActiveSheet()->getStyle('E4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->excel->getActiveSheet()->mergeCells('G3:J3');
			$this->excel->getActiveSheet()->setCellValue('G3', 'SISWA BARU');
			...
			$this->excel->getActiveSheet()->setCellValue('G4', 'TAMATAN TK');
			$this->excel->getActiveSheet()->getStyle('G4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->excel->getActiveSheet()->mergeCells('I4:J4');
			$this->excel->getActiveSheet()->setCellValue('I4', 'BUKAN TK');
			$this->excel->getActiveSheet()->getStyle('I4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		foreach($kecamatan as $dt_kecamatan)
		{
			...
			/*if($jenjang == 1)
			{				
				$jumTk = $this->mlaporan->getJumTk($dt_kecamatan->id_kecamatan);
				$this->excel->getActiveSheet()->setCellValue('C'.$baris, $jumTk['daftar_no_tk_lk']);
				$this->excel->getActiveSheet()->getStyle('C'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('D'.$baris, $jumTk['daftar_no_tk_pr']);
				$this->excel->getActiveSheet()->getStyle('D'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('E'.$baris, $jumTk['daftar_tk_lk']);
				$this->excel->getActiveSheet()->getStyle('E'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('F'.$baris, $jumTk['daftar_tk_pr']);
				$this->excel->getActiveSheet()->getStyle('F'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('G'.$baris, $jumTk['peserta_no_tk_lk']);
				$this->excel->getActiveSheet()->getStyle('G'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('H'.$baris, $jumTk['peserta_no_tk_pr']);
				$this->excel->getActiveSheet()->getStyle('H'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('I'.$baris, $jumTk['peserta_tk_lk']);
				$this->excel->getActiveSheet()->getStyle('I'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
				$this->excel->getActiveSheet()->setCellValue('J'.$baris, $jumTk['peserta_tk_pr']);
				$this->excel->getActiveSheet()->getStyle('J'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			}*/
			if(count($umur) > 0)
			{
		...
		/*$this->excel->getActiveSheet()->setCellValue($akhirJenjang.$baris, "=SUM(".$akhirJenjang."5:".$akhirJenjang.$baris.")");
		$this->excel->getActiveSheet()->getStyle($akhirJenjang.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);*/
	...
	function generate_rk2()
	{
		...
		$this->excel->getActiveSheet()->getStyle($akhir.'5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);		
		$nkolom = $nkolom + 2;
		/*$golongan = $this->arey->getStatusPeg();
		foreach($golongan as $key => $dt_golongan)
		{
			$awal = $huruf[$nkolom];
			$akhir = $huruf[$nkolom+1];
			$this->excel->getActiveSheet()->mergeCells($awal.'4:'.$akhir.'4');
			$this->excel->getActiveSheet()->setCellValue($awal.'4', $dt_golongan);
			$this->excel->getActiveSheet()->getStyle($awal.'4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);		
			$this->excel->getActiveSheet()->setCellValue($awal.'5', "L");
			$this->excel->getActiveSheet()->getStyle($awal.'5')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);		
			$this->excel->getActiveSheet()->setCellValue($akhir.'5', "P");
		...
		foreach($kecamatan as $dt_kecamatan)
		{
			...
			foreach($golongan as $key => $dt_golongan)
			{
				...
				$kepStatus = $this->mlaporan->getKepalaPendik($dt_kecamatan->id_kecamatan,$jenjang,$key);

				$this->excel->getActiveSheet()->setCellValue($awal.$baris, $kepStatus['laki']);
				$this->excel->getActiveSheet()->getStyle($awal.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);		
				$this->excel->getActiveSheet()->setCellValue($akhir.$baris, $kepStatus['pr']);
				$this->excel->getActiveSheet()->getStyle($akhir.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);			
				$keplaki = $keplaki + $kepStatus['laki'];
				$keppr = $keppr + $kepStatus['pr'];
				$nkolom += 2;
			}*/
	...
	function generate_sekolah($id)
	{
		...
		$brs_bwh = ($baris == 4)?$baris:$baris-1;
		$this->excel->getActiveSheet()->getStyle('A3:E'.$brs_bwh)->applyFromArray($styleArray);
		...
		/*$this->excel->getActiveSheet()->mergeCells('A'.$current.':A'.$current1);		
		$this->excel->getActiveSheet()->setCellValue('A'.$current, "Komputer/laptop TU");				
		$this->excel->getActiveSheet()->getStyle('A'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('B'.$current.':B'.$current1);
		$this->excel->getActiveSheet()->setCellValue('B'.$current, "Printer TU");				
		$this->excel->getActiveSheet()->getStyle('B'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('C'.$current.':C'.$current1);		
		$this->excel->getActiveSheet()->setCellValue('C'.$current, "Scanner");				
		$this->excel->getActiveSheet()->getStyle('C'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('D'.$current.':D'.$current1);		
		$this->excel->getActiveSheet()->setCellValue('D'.$current, "Digital Camera");				
		$this->excel->getActiveSheet()->getStyle('D'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('E'.$current.':E'.$current1);				
		$this->excel->getActiveSheet()->setCellValue('E'.$current, "Server");				
		$this->excel->getActiveSheet()->getStyle('E'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('F'.$current.':H'.$current);				
		$this->excel->getActiveSheet()->setCellValue('F'.$current, "Mesin");				
		$this->excel->getActiveSheet()->getStyle('F'.$current)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('F'.$current1, "Ketik");				
		$this->excel->getActiveSheet()->getStyle('F'.$current1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('G'.$current1, "Stensil");				
		$this->excel->getActiveSheet()->getStyle('H'.$current1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->setCellValue('H'.$current1, "Foto Kopi");			
		$this->excel->getActiveSheet()->getStyle('H'.$current1)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('I'.$current.':I'.$current1);			
		$this->excel->getActiveSheet()->setCellValue('I'.$current, "Brankas");				
		$this->excel->getActiveSheet()->getStyle('I'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('J'.$current.':J'.$current1);			
		$this->excel->getActiveSheet()->setCellValue('J'.$current, "Filling Kabinet/Almari");				
		$this->excel->getActiveSheet()->getStyle('J'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('K'.$current.':K'.$current1);			
		$this->excel->getActiveSheet()->setCellValue('K'.$current, "Meja TU");				
		$this->excel->getActiveSheet()->getStyle('K'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('L'.$current.':L'.$current1);			
		$this->excel->getActiveSheet()->setCellValue('L'.$current, "Kursi TU");				
		$this->excel->getActiveSheet()->getStyle('L'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('M'.$current.':M'.$current1);			
		$this->excel->getActiveSheet()->setCellValue('M'.$current, "Meja Guru");				
		$this->excel->getActiveSheet()->getStyle('M'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$this->excel->getActiveSheet()->mergeCells('N'.$current.':N'.$current1);			
		$this->excel->getActiveSheet()->setCellValue('N'.$current, "Kursi Guru");					
		$this->excel->getActiveSheet()->getStyle('N'.$current)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);*/
	...
	function generate_rk3()
	{
		...
		foreach($skulKec as $dt_kecamatan)
		{
			...
			$this->excel->getActiveSheet()->setCellValueExplicit('C'.$baris,  '12', PHPExcel_Cell_DataType::TYPE_STRING);
			$this->excel->getActiveSheet()->getStyle('C'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$jumSek = $this->mlaporan->getJumSekolah($dt_kecamatan->id_kecamatan,$jenjang);
			$this->excel->getActiveSheet()->setCellValue('D'.$baris, $jumSek);	
			$this->excel->getActiveSheet()->getStyle('D'.$baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
