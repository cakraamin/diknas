...
class Fasilitas extends CI_Controller {
	...
	function ruang()
	{
		...
		$data = array(	  		
			'main'			=> 'frmRuang',
			'ket'			=> 'Form Ruang Kelas Sekolah',
			'jenis'			=> 'Tambah',
			'fasilitas'		=> 'select',
			'link'			=> 'simpan_ruang/'.$this->session->userdata('id_school')."/".$ta_aktif['tahun'],
			'kueri'			=> $this->mfasilitas->getListRuang($this->session->userdata('id_school'),$ta_aktif['tahun'])
		);
	...
	function simpan_ruang($id,$thn)
	{
		$this->mfasilitas->addRuangSekolah($id,$thn);

		if($this->db->affected_rows() > 0)
		{
			$this->message->set('succes','Data ruang kelas sekolah berhasil ditambahkan');			
		}
		else
		{
			$this->message->set('notice','Data ruang kelas sekolah gagal ditambahkan');
		}

		redirect('fasilitas/ruang');
	}
