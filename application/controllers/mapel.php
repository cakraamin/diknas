...
class Mapel extends CI_Controller {
	...
	function index()
	{
		...
		$data = array(	  		
			...
			'rata'			=> $this->mmapel->getListMapelLain($ta_aktif['tahun'],$this->session->userdata('id_school')),
		);
	...
	{	
		...
		if($this->input->post('tingkate',TRUE) == 1)
		{
			foreach($kode as $dt_kode)
			{
				$this->mmapel->addDetailMapel($dt_kode,$ta_aktif['tahun']);
			}
		}
		else
		{
			$this->mmapel->addDetailMapelLain($ta_aktif['tahun']);
		}		
