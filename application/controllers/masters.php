...
class Masters extends CI_Controller {
	...
	function sekolah($id,$nama,$kec=0,$kunci="kosong",$short_by='id_school',$short_order='desc',$page=0)

	{

		$per_page = 10;

		$total_page = $this->mmaster->getNumSkul($id,$kec,$kunci);

		$url = 'masters/sekolah/'.$id.'/'.$nama.'/'.$kec.'/'.$kunci.'/'.$short_by.'/'.$short_order.'/';		

		$query = $this->mmaster->getSkul($kec,$kunci,$id,$per_page,$page,$short_by,$short_order);

		if(count($query) == 0 && $page != 0)

		{

			redirect('masters/sekolah/'.$id.'/'.$nama.'/'.$kec.'/'.$kunci.'/'.$short_by.'/'.$short_order.'/'.($page - $per_page));		
	...
	function fasilitas($jenjang=0,$short_by='id_fasilitas',$short_order='desc',$page=0)

	{

		$per_page = 10;		

		$total_page = $this->mmaster->getNumFasilitas($jenjang);

		$url = 'masters/fasilitas/'.$jenjang.'/'.$short_by.'/'.$short_order.'/';		

		$query = $this->mmaster->getFasilitas($jenjang,$per_page,$page,$short_by,$short_order);				

		if(count($query) == 0 && $page != 0)

		{

			redirect('masters/fasilitas/'.$jenjang.'/'.$short_by.'/'.$short_order.'/'.($page - $per_page));		

		}

		$tambah = array('0' => 'All');
		$jenjange = $this->arey->getJenjang();				
		$jenjangs = array_merge($tambah,$jenjange);		
	...
	function import_sekolah($id,$nama)

	{
		...
		$data = array(	  		
			...
			'ket'			=> 'Form Import Master Sekolah',

			'jenis'			=> 'Import',

			'sekolah'		=> 'select',

			'link'			=> 'upload_sekolah/'.$id.'/'.$nama,
	...
	function upload_sekolah($id,$nama="")
	{
		...
		else
		{
			...
			foreach($data['values'] as $value)
			{
				if($value['A'] != "")	
				{					
					if($this->mmaster->cekNpsnAuto($value['D']) == 0)
					{
						$data = array(
							'id_school'			=> '',
							'nama_school'		=> strip_tags(ascii_to_entities(addslashes($value['B']))),
							'email_school'		=> strip_tags(ascii_to_entities(addslashes($value['C']))),
							'npsn_school'		=> strip_tags(ascii_to_entities(addslashes($value['D']))),
							'jenjang_school'	=> $id
						);

						$this->mmaster->importSekolah($data);
					}					
				}
				$no++;
			}			
			unlink($uploadpath);
			$this->message->set('succes','Import Data Sekolah Berhasil');			
		}
	...
	{		
...

		{
			...
			redirect('masters/sekolah/'.$id.'/'.$nama.'/'.$kec.'/'.$kunci);			
	...
	function all_fasilitas()
	{
		...
		else
		{
			...
			redirect('masters/fasilitas/'.$jenjang);
		}
	}
