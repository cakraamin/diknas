...

{
	...
	function create($user_email = '', $user_pass = '', $level = '', $id = '', $auto_login = true) 
...

	{
...

		{
			...
			$user_data['levels'] = $user_data['user_level'];

			$user_data['id_school'] = $user_data['id_school'];
...

	{
		...
		return $this->CI->db->delete($this->user_table, array('id_school' => $school_id));
	...
	*/
	...
	public function cek_token($token)
	{
		...
		$this->CI->db->where('word', $token); 
		$query = $this->CI->db->get_where("captcha");
